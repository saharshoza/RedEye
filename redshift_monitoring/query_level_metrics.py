...
def run(thread_num,query_result_queue,db_queue,stop_thread,config_dict):
	...
	while (not stop_thread.is_set()):
		payload = {}
		df_response_query, error_query = thread_read.read_query_result(thread_num=thread_num,query_result_queue=query_result_queue,metric_name=metric_name,query_name='query3')
		df_response_scan, error_scan = thread_read.read_query_result(thread_num=thread_num,query_result_queue=query_result_queue,metric_name=metric_name,query_name='query6')

		if error_scan == -1 or error_query == -1:
